...
- **Iteration**: do not connect Claude Desktop to a live `tsx src/server.ts` while iterating on tool code. Use the MCP Inspector instead: `npx @modelcontextprotocol/inspector npx tsx src/server.ts`. The Inspector reloads tool definitions per call; Desktop caches them per session.
